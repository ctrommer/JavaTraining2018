...
public class ThreadMitCompletableFutureVerknuepfung {
	...
	public static void supplierInThreadZweiZusammengesetzt() {
		CompletableFuture<String> completableFuture
									= CompletableFuture
												.supplyAsync(()-> "Erster Teil aus supplyAssync")
												.thenCompose( 
														ergebnis 
														-> CompletableFuture.supplyAsync( 
																					()
																					->  ergebnis + " zweiter Teil aus thenCombine") );
		try {
			System.out.println( completableFuture.get() );
		} catch ( InterruptedException | ExecutionException e ) {
			e.printStackTrace();
	...
	public static void supplierInThreadMehrereParallel() {
		...
		CompletableFuture<Void> alleZusammenFuture = CompletableFuture.allOf( 
																		completableFuture1, 
																		completableFuture2, 
																		completableFuture3 );
		Stream<String> stream = Stream
									.of( completableFuture1, completableFuture2, completableFuture3 )
									.map( CompletableFuture::join );
		String alleErgebnisseGejoint = stream
